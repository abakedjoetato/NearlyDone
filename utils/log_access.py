...
async def get_sftp_log_file(server, log_type, max_retries=3):
    """
    ...
    temp_file = None
    retry_count = 0
    reduced_timeout = max(SSH_TIMEOUT // 2, 10)  # Half the normal timeout for faster failure
    ...
    while retry_count <= max_retries:
        try:
            ...
            # Adjust timeout based on retry count (use a shorter timeout for retries to fail faster)
            current_timeout = reduced_timeout if retry_count > 0 else SSH_TIMEOUT
                ...
                **auth_options,
                ...
                async with conn.start_sftp_client() as sftp:
                    try:
                        ...
                        return None  # No need to retry if files don't exist
                        
                    except Exception as e:
                        logger.error(f"Error accessing files in server directory: {e}")
                        # We'll let this exception propagate to the retry logic
                        raise
            
            # If we get here, the connection and download succeeded
            return temp_file
            ...
            if retry_count <= max_retries:
                ...
                logger.warning(f"SSH/SFTP error: {e}, retrying in {wait_time}s (attempt {retry_count}/{max_retries})")
                await asyncio.sleep(wait_time)
            else:
                logger.error(f"SSH/SFTP error after {max_retries} retries: {e}")
                return None
            ...
            if retry_count <= max_retries:
                ...
                logger.warning(f"Error in SFTP file access: {e}, retrying in {wait_time}s (attempt {retry_count}/{max_retries})")
                await asyncio.sleep(wait_time)
            else:
                logger.error(f"Error in SFTP file access after {max_retries} retries: {e}")
                return None
            
        finally:
            # Only clean up temp file on the last try
            if retry_count > max_retries and temp_file and not os.path.exists(temp_file):
                try:
                    os.unlink(temp_file)
                except:
                    pass
...
async def get_sftp_all_csv_files(server, max_retries=3):
    """
    ...
    while retry_count <= max_retries:
        try:
            ...
            if not temp_dir:
                ...
                temp_files = []  # Reset temp files list when creating a new dir
                ...
                **auth_options,
                ...
                async with conn.start_sftp_client() as sftp:
                    try:
                        ...
                        # If still no CSV files, return empty result but don't retry (nothing to download)
                        if not csv_files:
                        ...
                        # If all downloads failed, we should retry
                        if download_failures == len(csv_files):
                            raise Exception(f"All {download_failures} file downloads failed")
                        
                        if not temp_files:
                            logger.warning(f"No CSV files downloaded for server {server.name}")
                            if temp_dir:
                                try:
                                    os.rmdir(temp_dir)  # Clean up empty dir
                                except:
                        ...
                        # Let the exception propagate to the retry logic
                        raise
            ...
            if retry_count <= max_retries:
                ...
                logger.error(f"SSH/SFTP error after {max_retries} retries: {e}")
                # Clean up any temp files on final failure
                if temp_files:
            ...
            if retry_count <= max_retries:
                wait_time = 2 ** retry_count
                logger.warning(f"Error in SFTP batch file access: {e}, retrying in {wait_time}s (attempt {retry_count}/{max_retries})")
                await asyncio.sleep(wait_time)
            else:
                logger.error(f"Error in SFTP batch file access after {max_retries} retries: {e}")
                # Clean up any temp files on final failure
                if temp_files:
                    for file in temp_files:
                        try:
